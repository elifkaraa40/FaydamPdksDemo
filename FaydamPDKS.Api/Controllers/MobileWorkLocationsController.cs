...
public sealed class MobileWorkLocationsController(IWorkLocationService service,TimeProvider clock):ControllerBase
{
 ...
 [HttpPost("requests"), HttpPost("field-requests")]
 public async Task<IActionResult> Create(CreateFieldWorkRequestDto r,CancellationToken ct)
 {
  if(!UserId(out var id))return Unauthorized();
  try
  {
   await service.CreateFieldRequestAsync(id,r,ct);
   return Accepted();
  }
  catch(WorkLocationOverlapException ex)
  {
   return Conflict(new ApiErrorDto(
    "WORK_LOCATION_OVERLAP",
    "Seçilen tarih aralığında bekleyen veya onaylanmış başka bir izin ya da çalışma konumu kaydı bulunmaktadır.",
    new Dictionary<string,string[]>
    {
     ...
     ["conflictingRecordType"]=[ex.ConflictingRecordType]
    },
    HttpContext.TraceIdentifier));
  }
  catch(InvalidOperationException ex)
  {
   return BadRequest(new ApiErrorDto("WORK_LOCATION_REQUEST_REJECTED",ex.Message,TraceId:HttpContext.TraceIdentifier));
  }
 }
