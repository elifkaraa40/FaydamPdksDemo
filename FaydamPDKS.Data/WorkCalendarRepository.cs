...
public sealed class WorkCalendarRepository(AppDbContext context) : IWorkCalendarRepository
{
    ...
    public Task<WorkCalendarDay?> GetAsync(Guid? workplaceId, DateOnly date, CancellationToken cancellationToken = default) =>
        context.WorkCalendarDays.SingleOrDefaultAsync(x => x.WorkplaceId == workplaceId && x.Date == date, cancellationToken);
